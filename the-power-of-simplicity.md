...
comments:
  - author: trevharmon
    ...
    comment: |
      ...
      - Everyone else is doing it &mdash; - That argument didn't fly when I was a kid, so I don't see why it's valid now.
      - Customers are demanding new features &mdash; - Maybe they are, but that doesn't necessarily mean those features should be enabled if they increase complexity. In my experience it's the edge cases that create the most complexity.
      - Flexibility is more important than usability &mdash; - This is a philosophical debate, and I admit I fall into different camps depending on the circumstance. However, I'm always for simplicity.
      - We prove how smart we are by making something complex &mdash; - No one ever says this one out loud, though I've seen it implied a number of times. There's an unfounded fear the finesse of a simple approach will hide the creator's brilliance. Oddly, usable simplicity is far more difficult to achieve than complexity. Anyone can make something more complex.
  ...
  - author: Daniel Hardman
    ...
    comment: |
      I hadn't thought about it in quite those terms before, Andy, but you're totally right. One more reason for Didgets, which in some ways has the same vision as the old "winfs" idea that MS used to talk about &mdash; describe data in robust and general-purpose ways that provide efficient searching &mdash; without imposing lots of burden on the data due to the particular application that's using it.
---
...
The software industry desperately needs this sort of insight, but far too often I see us operate in the stage of the narrative where the pupil misunderstands the problem, struggles, and makes things worse. This is true of all sorts of software &mdash; even mobile apps and sumer web sites &mdash; but I'm especially thinking about enterprise stacks. The architectures that I encounter today are significantly more complex than the ones I drew on whiteboards a decade ago. Applications used to consist of a binary and a handful of config files. Now they consist of hundreds or thousands of artifacts: executables, shared libraries, plugins, parsers, databases, triggers, stored procedures, data sets, documentation, brandable CSS, image libraries, drivers, rule sets, comm channels... Products have more features &mdash; <em>way</em> more. We sneer at offerings that aren't <em>solutions</em>. We build federated, distributed, loosely coupled fabrics that run in sophisticated clouds and that abstract physical geography, hardware, network interconnects.
