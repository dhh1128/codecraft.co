...
- [x] Fix the 2 malformed image URLs (`staticfliccom`, `static.flickr.com`) — both became owned AI art in the Flickr pass; malformed count now 0
- [~] Fill `disposition` per image — 44 Flickr → `generate` (done), 82 → `own` (done); remaining blanks are the 20 xkcd comics + ~4 one-offs (separate drop/attribute decision)
- [x] Establish a house visual style for AI-generated art → `docs/art-style.md` (editorial line illustration; brand-brown linework + terracotta accent, transparent background) + per-image prompts in `assets/art-prompts.yml`
- [x] **Replace all 44 unowned Flickr photos with owned editorial-line art** (openart.ai). Pipeline: `art-prompts.yml` → generate → `normalize_art.py` (transparent + 1024px + optimized) → `ingest_art.py` (map-by-content, localize, strip false credit + stale dims, CREDITS, 0644). External `<img>` 66→24 (remaining = xkcd + one-offs).
- [x] `normalize_art.py` ↔ test (5): background knockout (exterior-only, enclosed fills survive) + downscale-to-1024 + optimize
- [x] `ingest_art.py` ↔ test (6): copy→assets, normalize, rewrite `<img src>`, strip `(Flickr)` caption credit, strip stale width/height, CREDITS, mark done, delete download
- [~] `apply_image_triage.py` ↔ test `tests/test_apply_image_triage.py` (9 tests): `own` disposition implemented (localize + download, no-op-aware, path-normalizing). `redraw`/`drop` + reflow still to do.
...
- [~] Record provenance/rights for every image in `assets/CREDITS.yml` — 15 Wikimedia entries + 44 AI-generated entries (origin/model/prompt/owned). Remaining: entries for the wp/local owned images. Follow-up: 8 recovered Wikimedia images are CC-BY/SA/GFDL (attribution-required) — decide visible-attribution treatment (figcaption vs. a credits page).
