...
STD_LINK_PATTERN = re.compile(r'https?://codecraft.co/(\d{4}/\d{2}/\d{2})/(.+)')
UPLOAD_LINK_PATTERN = re.compile(r'https?://codecraft.co/wp-content/uploads/(\d{4}/\d{2}(?:/\d{2})?)/(.+)')
DRAFT_LINK_PATTERN = re.compile(r'https?://codecraft.co/\?p=(\d+)')
